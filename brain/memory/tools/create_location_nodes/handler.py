...
async def execute(args: dict[str, Any], context: dict[str, Any]) -> str:
    ...
    try:
        with kg_manager.driver.session() as session:
            node_id = kg_manager.create_location_node(
                session=session,
                name=name,
        ...
        if not node_id:
            return format_json({"success": False, "error": "创建地点节点失败"})

        return format_json({"success": True, "node_id": node_id, "node_type": "Location"})
    except Exception as exc:
        return format_json({"success": False, "error": f"创建地点节点异常: {exc}"})
